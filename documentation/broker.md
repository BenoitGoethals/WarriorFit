...
The broker is constructed by the DI container and `Broker.start()` is called from `app.py` immediately after wiring:
...
|---|---|---|
| Loop already running at import time | Creates `_worker_task` via `loop.create_task(self.worker())`, sets `running=True`, returns `True` | Worker runs every `poll_interval_s`, drains queue & sends to HR |
| **No loop yet** (module-import path) | Catches the `RuntimeError`, logs a `WARNING`, leaves `running=False` and `_worker_task=None`, returns `False` | The first `await broker.send_message(test)` calls `self.start()` again — it succeeds (we're now inside the loop) and the worker launches |
| Already running | Logs a `WARNING` ("already running"), returns `True` | No-op — existing worker keeps running |

The lazy-start path means **no message is ever lost just because the eager start ran before the loop**. The message lands on `_msg_queue` and the worker (started by the same `send_message`) drains it on its next cycle.
