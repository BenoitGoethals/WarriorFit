...
class RetentionService(Service):
    ...
    async def _purge_fitness(self) -> int:
        ...
        try:
            async with self.SessionLocal() as session, session.begin():
                ...
                if not ids:
                    return 0
                result = await session.execute(delete(FitnessTest).where(FitnessTest.id.in_(ids)))
    ...
    async def _purge_hr_messages(self) -> int:
        cutoff = datetime.now() - timedelta(days=self._retention["hr_message_retention_days"])
    ...
    async def _delete_older_than(self, model, date_column, cutoff) -> int:
        try:
            async with self.SessionLocal() as session, session.begin():
                result = await session.execute(delete(model).where(date_column < cutoff))
                return result.rowcount or 0
        except SQLAlchemyError as e:
