...
class MarsRepository(ABCRepository):
            ...
            async with self.SessionLocal() as session:
                async with session.begin():
                    ...
                    if result.rowcount == 0:
                        self._logger.error(f"No mars found with ID {ind_mars}.")
                        return False
                    return True
        except SQLAlchemyError as e:
            self._logger.exception("Failed to delete mars")
            if session is not None:
                try:
                    await session.rollback()
                    return False
    ...
    async def update_mars(self, mars: Mars) -> Mars | None:
        ...
        try:
            ...
            self._logger.exception(f"Failed to update mars with ID {mars.id}: {str(e)}")
            if session is not None:
                try:
                    await session.rollback()
