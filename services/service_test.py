...
class ServiceTest(Service):
    ...
    async def add_fitness_test_to_TestSession(self, param, cp):
        ...
        if add_test:
            await self.add_audit_log(details=f"Fitness test {cp.serial_number} added to test session {param}",action="add")
        return add_test
    ...
    async def update_fitness_test(self, param, cp):
        ...
        if updated:
            await self.add_audit_log(details=f"Fitness test {cp.serial_number} updated in test session {param}",action="update")
        return updated
    ...
    async def add_test_session(self, ts):
        added_test= await self._test_repo.add_test_session(ts)
        if added_test:
            await self.add_audit_log(details=f"Test session {ts.id} added",action="add")
        return added_test

    async def update_test_session(self, data):
        updated= await self._test_repo.update_test_session(data)
        if updated:
            await self.add_audit_log(details=f"Test session {data.id} updated",action="update")
        return updated
