...
class TypeFitnessTest(Enum):
    ...
    SWIMMING = "SWIMMING"
    MFFT_EVAL = "MFFT_EVAL"   # added 2026 for the Eval MFFT (see below)
    ENDURANCE = "ENDURANCE"   # ← new
```

> **Reference implementation** — the `MFFT_EVAL` test type added in 2026 is
> the most complete worked example of this guide: 8 result columns,
> cluster-aware scoring (`MfftEvalCalculator`), 30 unit tests, dashboard +
> analytics integration, CSV / PDF reports, polymorphic loading in 8
> repository queries. Browse `warriorfit/data/model/db_model.py` →
> `MfftEvalTest`, `warriorfit/logic/mfft_eval_calculator.py`,
> `warriorfit/ui/pages/mfft_eval.py` and the
> `f6a7b8c9d0e1_add_mfft_eval_test_and_cluster.py` migration when in doubt.
...
> **Postgres enum gotcha** — if you store the new test type on a column
> mapped to `SAEnum(TypeFitnessTest)` (e.g. `TestSession.type_test`), the PG
> enum type already exists and Alembic will not extend it for you. Hand-edit
> the migration to `op.execute("ALTER TYPE typefitnesstest ADD VALUE IF NOT
> EXISTS 'YOUR_NEW_VALUE'")`. The `f6a7b8c9d0e1_add_mfft_eval_test_and_cluster.py`
> migration shows the exact pattern.
