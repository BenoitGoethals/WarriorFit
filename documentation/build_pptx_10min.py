...
def tb(
    slide, text, x, y, w, h, size=18, bold=False, color=C_WHITE, align=PP_ALIGN.LEFT, italic=False
):
...
def tb_lines(slide, lines, x, y, w, h, size=16, color=C_WHITE, align=PP_ALIGN.LEFT, sa=6, lc=None):
    box = slide.shapes.add_textbox(x, y, w, h)
    tf = box.text_frame
    tf.word_wrap = True
    for i, line in enumerate(lines):
        ...
        r.font.size = Pt(size)
        r.font.name = "Calibri"
        r.font.color.rgb = lc[i] if isinstance(lc, list) and i < len(lc) else lc if lc else color
    return box
...
def bullet(slide, items, x, y, w, h, size=18, bc=C_AMBER, tc=C_WHITE, sa=8):
    ...
    for i, item in enumerate(items):
        ...
        if isinstance(item, tuple):
            rt.text, rt.font.color.rgb = item[0], item[1]
        else:
            rt.text = item
...
tb(
    s,
    "Digitalisering van militaire fysieke fitheidstesten",
    Inches(1),
    Inches(3.75),
    Inches(11.33),
    Inches(0.65),
...
s = prs.slides.add_slide(blank)
set_bg(s, C_BG)
title_bar(s, "Probleemstelling", "Manuele, papieren registratie van militaire fitheidstesten")
...
bullet(
    s,
    [
        "Papieren fiches — geen centraal overzicht",
...
tb(
    s,
    "→  WarriorFit digitaliseert het volledige proces",
    cx2 + Inches(0.2),
    Inches(5.9),
    cw - Inches(0.3),
    Inches(0.75),
...
set_bg(s, C_BG)
title_bar(
    s, "Oplossing — WarriorFit Ecosysteem", "Intranetwebplatform · geen publieke blootstelling"
)
kpi_row(
    s,
    [
...
img.width = int(img.width * (Inches(4.5) / img.height) if img.height > 0 else img.width)
img.left = Inches(0.3)
...
title_bar(
    s,
    "Architectuur — Gedetailleerde lagenstructuur",
...
s = prs.slides.add_slide(blank)
set_bg(s, C_BG)
title_bar(s, "Data Model & ERD", "PostgreSQL · SQLAlchemy ORM · polymorf FitnessTest")
...
set_bg(s, C_BG)
title_bar(
    s, "Rollen & Toegangscontrole (RBAC)", "6 rollen · PageSpec(allowed_roles=[…]) per module"
)
add_tbl(
    s,
    ...
    Inches(7.8),
    Inches(4.7),
    cw=[Inches(1.6), Inches(2.5), Inches(3.7)],
    hs=15,
    rs=14,
)
rx2 = Inches(8.5)
...
title_bar(
    s,
    "5 Testtypes — volledig gedigitaliseerd",
...
for i, (name, color, desc) in enumerate(tests):
    ...
    tb(s, name, bx, by, bw, Inches(0.52), size=18, bold=True, color=C_BG, align=PP_ALIGN.CENTER)
...
tb(
    s,
    "Zijbrugscoringtabel (PHEF) — leeftijds- en gendercorrectie:",
    Inches(0.3),
    Inches(4.95),
    Inches(5.5),
    Inches(0.38),
...
tb_lines(
    s,
    [
        "Gemeenschappelijk voor alle types:",
...
title_bar(
    s,
    "Dataflow — PHEF-invoer & Broker (MOM)",
...
tb_lines(
    s, sync_steps, Inches(0.6), Inches(1.85), Inches(5.4), Inches(3.1), size=15, color=C_WHITE, sa=9
)

tb(
    s,
    "Asynchroon pad  (broker — PTI wacht NIET)",
    Inches(0.4),
    Inches(5.2),
    Inches(5.8),
    Inches(0.4),
...
# ===========================================================================
image_slide(
    "Volledige Testflow — UI → Service → DB → Broker → HR",
...
set_bg(s, C_BG)
title_bar(
    s, "Agile Methodologie — Scrum metrics", "Sep 2025 – mei 2026 · 1 developer · 2-weekse sprints"
)
kpi_row(
    s,
    [
...
set_bg(s, C_BG)
title_bar(
    s, "Kwaliteit & Testing", "pytest-asyncio · mypy strict · ruff · pre-commit · GitHub Actions"
)
...
set_bg(s, C_BG)
title_bar(
    s, "Beveiliging — OWASP Top 10 evaluatie", "Defense-in-depth · intranet-deployment · Argon2id"
)
tbl = add_tbl(
    s,
    ...
    [
        (
            ...
            "RBAC per PageSpec · IDOR-fix · sessie-scoped UserStore",
            "✓",
        ),
    ...
    Inches(12.6),
    Inches(5.85),
    cw=[Inches(3.0), Inches(7.3), Inches(2.3)],
    hs=14,
    rs=13,
)
...
set_bg(s, C_BG)
title_bar(
    s, "AVG / GDPR Compliance", "DPIA Art. 35 · gezondheidsdata Art. 9 · zelfservice privacypagina"
)
for cx, title_t, items in [
    (
        Inches(0.4),
    ...
    bullet(s, items, cx + Inches(0.2), Inches(2.0), Inches(5.6), Inches(4.8), size=14, sa=9)
...
s = prs.slides.add_slide(blank)
set_bg(s, C_BG)
title_bar(s, "Technologiestapel", "Python 3.13+ · moderne async stack · volledig open source")
groups = [
    (
        "Frontend / UI",
        C_AMBER,
...
s = prs.slides.add_slide(blank)
set_bg(s, C_BG)
title_bar(s, "Routekaart — verdere ontwikkeling", "Open punten & toekomstige iteraties")
items_rk = [
    (
        C_RED,
        "Hoog",
...
tb(
    s,
    "Benoit Goethals  ·  benoit.goethals@gmail.com",
    Inches(1),
    Inches(4.0),
    Inches(11.33),
    Inches(0.5),
...
tb(
    s,
    "github.com/BenoitGoethals/WarriorFit",
    Inches(1),
    Inches(4.5),
    Inches(11.33),
    Inches(0.45),
